...
$sshArgs = @('-o','StrictHostKeyChecking=accept-new','-o',"UserKnownHostsFile=$($GlobalLabConfig.SSH.KnownHostsPath)",'-o',"ConnectTimeout=$GlobalLabConfig.Timeouts.Linux.SSHConnectTimeout",'-i',(Join-Path (Join-Path $GlobalLabConfig.Paths.LabSourcesRoot SSHKeys) id_ed25519),"$GlobalLabConfig.Credentials.LinuxUser@$ip")
...
try {
    & $scpExe -o StrictHostKeyChecking=accept-new -o "UserKnownHostsFile=$($GlobalLabConfig.SSH.KnownHostsPath)" -i (Join-Path (Join-Path $GlobalLabConfig.Paths.LabSourcesRoot SSHKeys) id_ed25519) $tempScript "${LinuxUser}@${ip}:/tmp/install-ansible.sh" 2>&1 | Out-Null
...
if (Test-Path $inventoryTemplate) {
    ...
    & $scpExe -o StrictHostKeyChecking=accept-new -o "UserKnownHostsFile=$($GlobalLabConfig.SSH.KnownHostsPath)" -i (Join-Path (Join-Path $GlobalLabConfig.Paths.LabSourcesRoot SSHKeys) id_ed25519) $inventoryPath "${LinuxUser}@${ip}:~/ansible/inventory/lab.yml" 2>&1 | Out-Null
...
if (Test-Path $playbooksDir) {
    ...
    foreach ($pb in $playbooks) {
        & $scpExe -o StrictHostKeyChecking=accept-new -o "UserKnownHostsFile=$($GlobalLabConfig.SSH.KnownHostsPath)" -i (Join-Path (Join-Path $GlobalLabConfig.Paths.LabSourcesRoot SSHKeys) id_ed25519) $pb.FullName "${LinuxUser}@${ip}:~/ansible/playbooks/$($pb.Name)" 2>&1 | Out-Null
    }
